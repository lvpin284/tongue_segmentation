...
class JointTransform2D:
    ...
    def __init__(self, img_size=256, low_img_size=256, ori_size=256, crop=(32, 32), p_flip=0.0, p_rota=0.0, p_scale=0.0,
    ...
    def __call__(self, image, mask, prior=None):
        """Augment image/mask, optionally applying the same geometric transforms to
        a multi-channel ``prior`` map (H, W, C) so it stays spatially aligned with
        the augmented image. Photometric ops (gamma/noise/contrast/color) are applied
        to the image only. When ``prior`` is given, returns a 4-tuple with an extra
        prior tensor of shape (C, img_size, img_size); otherwise returns the original
        3-tuple for backward compatibility.
        """
        has_prior = prior is not None
        if has_prior:
            # (H, W, C) float32 in [0, 1] -> tensor (C, H, W) so torchvision functional
            # transforms can share the same geometric parameters as image/mask.
            prior_t = torch.as_tensor(np.ascontiguousarray(prior), dtype=torch.float32).permute(2, 0, 1)
        ...
        if np.random.rand() < self.p_scale:
            ...
            if has_prior:
                prior_t = F.resize(prior_t, (new_h, new_w), InterpolationMode.BILINEAR)
        ...
        if np.random.rand() < self.p_random_affine:
            ...
            if has_prior:
                prior_t = F.affine(prior_t, *affine_params)
        ...
        if has_prior:
            prior_t = F.resize(prior_t, (self.img_size, self.img_size), InterpolationMode.BILINEAR)
            prior_t = prior_t.clamp(0.0, 1.0)
            return image, mask, low_mask, prior_t
...
class ImageToImage2D(Dataset):
    ...
    def __init__(self, dataset_path: str, split='train', joint_transform: Callable = None, img_size=256, cls_sim_arr=None, prompt="click",
                 dict_avg_label=None, class_id=1,
        ...
        self.prompt = prompt
        self.img_size = img_size
        self.cls_sim_arr = cls_sim_arr
        self.dict_avg_label = dict_avg_label
    ...
    def __getitem__(self, i):
        ...
        filename_index = np.where(self.cls_sim_arr == filename_png)[0][0]
        if filename_index >= 0:
            cls_sim = self.cls_sim_arr[filename_index, 1:].astype(np.float32).reshape(1, -1)
        else:
            raise ValueError(f"Element {filename_png} not found in the array")

        if self.dict_avg_label is not None:
            if 'train' in self.split:
                class_id = int(class_id0)
            elif 'val' in self.split:
                class_id = int(class_id0)
            ...
            avg_label_cluster1 = self.dict_avg_label[f'avg_label_cluster1']
            avg_label_cluster2 = self.dict_avg_label[f'avg_label_cluster2']
            avg_label_cluster3 = self.dict_avg_label[f'avg_label_cluster3']
            avg_label_cluster4 = self.dict_avg_label[f'avg_label_cluster4']

            avg_label_cluster1 = avg_label_cluster1 * cls_sim[0, 0]
            avg_label_cluster2 = avg_label_cluster2 * cls_sim[0, 1]
            avg_label_cluster3 = avg_label_cluster3 * cls_sim[0, 2]
            avg_label_cluster4 = avg_label_cluster4 * cls_sim[0, 3]

            cls_sim_avg_label_input = np.stack([avg_label_cluster1, avg_label_cluster2, avg_label_cluster3, avg_label_cluster4], axis=-1).transpose(2, 0, 1)
