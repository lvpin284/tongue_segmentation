    ...
    def __init__(
        ...
        for i in range(depth):
            block = ParaBlock(
                ...
                window_size=window_size if i not in global_attn_indexes else 0,  # 14或者0,global_attn_indexes[2,5,8,11]
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        ...
        for blk in self.blocks:
            ...
            if blk.window_size == 0:
                cnn_feature_list.append(self.neck(cnnx.permute(0, 3, 1, 2)))

        x = x + 0.5 * cnnx  # bs 32 32 768

        x = self.neck(x.permute(0, 3, 1, 2))    # bs 256 32 32

        return x, cnn_feature_list
...
class SingleDown_ResNet(nn.Module):
    """Downscaling with maxpool then double conv with residual connection"""
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3):
        ...
        self.residual = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=2, bias=False)
...
class SingleConv_ResNet(nn.Module):
    """Double conv with residual connection"""
...
class SingleCNNEmbed(nn.Module):
    ...
    def __init__(
        ...
        downtimes = int(math.log2(patchsize))
        mid_channel = 64
        self.inc = SingleConv_ResNet(in_chans, mid_channel)
        self.downs = nn.ModuleList()
        for i in range(downtimes):
            if i == downtimes - 1:
                down = SingleDown_ResNet(mid_channel, embed_dim)
            else:
                down = SingleDown_ResNet(mid_channel, mid_channel * 2)
            mid_channel = mid_channel * 2
            self.downs.append(down)
